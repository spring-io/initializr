...
public class Dependency extends MetadataElement implements Describable {
     ...
     * All scope types.
     */
    public static final List<String> SCOPE_ALL = List.of(SCOPE_COMPILE, SCOPE_RUNTIME, SCOPE_COMPILE_ONLY, SCOPE_ANNOTATION_PROCESSOR, SCOPE_PROVIDED, SCOPE_TEST);
    ...
    public void updateCompatibilityRange(VersionParser versionParser) {
        if (this.compatibilityRange != null) {
            try {
                ...
                throw new InvalidInitializrMetadataException("Invalid compatibility range '" + this.compatibilityRange + " for " + "dependency with id '" + getId() + "'", ex);
            }
        }
        this.mappings.forEach((it) -> {
            try {
                Assert.notNull(it.compatibilityRange, "Invalid mapping for " + this + ": compatibility range is mandatory");
