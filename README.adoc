...
`name`, `description`, `packageName`, `packaging`, `javaVersion`, `language` and `dependencies`.
Use a comma-separated list of dependency ids for `dependencies` like `dependencies=security,aop`.
