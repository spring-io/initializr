...
public class GroovyDslGradleBuildWriter extends GradleBuildWriter {
    ...
    private void writeBuildscriptDependencies(IndentingWriter writer, GradleBuild build) {
        writeNestedCollection(writer, "dependencies", build.getBuildscript().getDependencies(), (dependency) -> "classpath '" + dependency + "'");
    }
    ...
    protected void writePlugins(IndentingWriter writer, GradleBuild build) {
        ...
        writeCollection(writer, extractApplyPlugins(build), (plugin) -> "apply plugin: '" + plugin.getId() + "'", writer::println);
        writer.println();
    }
    ...
    protected void writeExtraProperties(IndentingWriter writer, Map<String, String> allProperties) {
        writeNestedCollection(writer, "ext", allProperties.entrySet(), (e) -> getFormattedExtraProperty(e.getKey(), e.getValue()), writer::println);
    }
    ...
    protected void writeConfiguration(IndentingWriter writer, GradleConfiguration configuration) {
        ...
        writer.indented(() -> writer.println(String.format("extendsFrom %s", String.join(", ", configuration.getExtendsFrom()))));
        writer.println("}");
    }
    ...
    protected String bomAsString(BillOfMaterials bom) {
        ...
        return "mavenBom " + quoteStyle + bom.getGroupId() + ":" + bom.getArtifactId() + ":" + determineVersion(bom.getVersion()) + quoteStyle;
    }
    ...
    protected String invocationAsString(GradleTask.Invocation invocation) {
        String arguments = (invocation.getArguments().isEmpty()) ? "()" : " " + String.join(", ", invocation.getArguments());
