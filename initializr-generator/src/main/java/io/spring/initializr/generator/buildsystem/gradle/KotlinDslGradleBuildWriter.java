...
public class KotlinDslGradleBuildWriter extends GradleBuildWriter {
    ...
    private String getJavaVersionConstant(String jvmVersion) {
        return this.sourceCompatibilitiesToJavaVersion.computeIfAbsent(jvmVersion, (key) -> {
            ...
            int generation = (jvmVersion.startsWith("1.") ? Integer.parseInt(jvmVersion.substring(2)) : Integer.parseInt(jvmVersion));
    ...
    protected void writeConfigurations(IndentingWriter writer, GradleConfigurationContainer configurations) {
        ...
        writer.indented(() -> configurations.customizations().forEach((configuration) -> writeConfiguration(writer, configuration, customConfigurations)));
        writer.println("}");
        writer.println("");
    }
    ...
    protected void writeConfiguration(IndentingWriter writer, GradleConfiguration configuration, List<String> customConfigurations) {
        if (configuration.getExtendsFrom().isEmpty()) {
            ...
            writer.println(configuration.getName() + " {");
            writer.indented(() -> writer.println(String.format("extendsFrom(%s)", configuration.getExtendsFrom().stream().map((name) -> configurationReference(name, customConfigurations)).collect(Collectors.joining(", ")))));
            writer.println("}");
        }
    }
    ...
    protected void writeDependency(IndentingWriter writer, Dependency dependency) {
        ...
        writer.print(configurationForDependency(dependency) + "(\"" + dependency.getGroupId() + ":" + dependency.getArtifactId() + ((version != null) ? ":" + version : "") + ((classifier != null) ? ":" + classifier : "") + ((type != null) ? "@" + type : "") + "\")");
        if (!dependency.getExclusions().isEmpty()) {
            writer.println(" {");
            writer.indented(() -> writeCollection(writer, dependency.getExclusions(), this::dependencyExclusionAsString));
            writer.println("}");
        } else {
            writer.println();
        }
    ...
    protected void writeExtraProperties(IndentingWriter writer, Map<String, String> allProperties) {
        writeCollection(writer, allProperties.entrySet(), (e) -> getFormattedExtraProperty(e.getKey(), e.getValue()), writer::println);
    }
