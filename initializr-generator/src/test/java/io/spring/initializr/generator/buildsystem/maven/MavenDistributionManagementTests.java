...
class MavenDistributionManagementTests {
    ...
    void distributionManagementWithRepository() {
        MavenDistributionManagement mdm = builder().repository((repository) -> repository.id("released-repo").name("released repo").url("https://upload.example.com/releases")).repository((repository) -> repository.layout("default")).build();
    ...
    void distributionManagementWithSnapshotRepository() {
        MavenDistributionManagement mdm = builder().snapshotRepository((repository) -> repository.id("snapshot-repo").name("snapshot repo").url("scp://upload.example.com/snapshots")).snapshotRepository((repository) -> repository.uniqueVersion(true)).build();
    ...
    void distributionManagementWithSite() {
        MavenDistributionManagement mdm = builder().site((site) -> site.id("website").name("web site")).site((site) -> site.url("scp://www.example.com/www/docs/project")).build();
