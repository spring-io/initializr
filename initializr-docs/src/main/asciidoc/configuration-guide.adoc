...
to a project. The `SampleContributor` registered above generates a `hello.txt` file at
...
A `ProjectDescriptionDiff` only reveals *that* an attribute changed, which leaves other
extensions to guess *why*. To avoid such guesses, a `ProjectDescriptionCustomizer` can record
the reason for a change it makes using the `ProjectDescriptionChanges` of the description, as
shown below:

[source,java,indent=0,subs="verbatim,quotes,attributes"]
----
include::{code-examples}/doc/generator/project/ProjectCustomizationExamples.java[tag=jvm-version-change-reason]
----

A reason is reported as-is, so it should be a self-contained sentence, and it must only be
recorded if the attribute is actually changed. The default help document turns each recorded
reason into a warning, that is a bullet point of the `Read Me First` section of `HELP.md`.
As `HELP.md` is a Markdown document, a reason that is rendered by the default help document
can use Markdown, typically to link to a page with more details.

At most one reason is kept per attribute: if several customizers change the same attribute, only
the reason of the last one is kept as it supersedes the previous ones. A reason should therefore
describe the value that has been set and why, but not the value that has been replaced, as the
latter may well be an intermediate value that no other component has seen.

Each `MutableProjectDescription` has its own `ProjectDescriptionChanges`. Customizers are
typically singletons of the parent application context and are therefore shared by concurrent
project generations, so a customizer must always record against the description it is given and
must never hold on to a `ProjectDescriptionChanges` instance.
...
`section`, and a `src/{sourceSet}/resources/application[-{profile}].{properties|yaml}`
file, resolved against the project's build system, is generated for every section that
contains properties, as shown in the following example:
...
The `bootVersion` parameter also accepts an abbreviated form, where the minor and patch
numbers are either omitted or specified as `x`. The service then selects the latest
generally available version that matches, ignoring milestones and snapshots. `4` and
`4.x.x` both resolve to the latest generally available `4.x` version, and `4.0` and `4.0.x`
both resolve to the latest generally available `4.0` version:

[source]
----
$ curl -G http://localhost:8080/starter.zip -d bootVersion=4.0.x -o my-project.zip
----

The same forms are supported by the `/dependencies` endpoint.

Only the trailing numbers can be replaced by `x`, and an abbreviated form cannot be
combined with a qualifier. Values such as `4.x.3`, `4.0.x-M1` or `4.x.x-SNAPSHOT` are
therefore rejected. Any other value is used as is, so `4.0.2-M1` selects that exact
version.

The request is rejected with a `400` response if no available version matches the
abbreviated form, or if the version it resolves to is outside the platform compatibility
range that the service is configured with. In the latter case, the error message refers to
the resolved version rather than the abbreviated form that was requested.
