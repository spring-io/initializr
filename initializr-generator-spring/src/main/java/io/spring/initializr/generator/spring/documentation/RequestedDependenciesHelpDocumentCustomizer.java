...
public class RequestedDependenciesHelpDocumentCustomizer implements HelpDocumentCustomizer {
    ...
    public RequestedDependenciesHelpDocumentCustomizer(ProjectDescription description, InitializrMetadata metadata) {
        ...
        this.platformVersion = (description.getPlatformVersion() != null) ? description.getPlatformVersion().toString() : metadata.getBootVersions().getDefault().getId();
    }
    ...
    private void handleDependency(HelpDocument document, Dependency dependency) {
        ...
        registerLinks(indexedLinks.get(GuideType.REFERENCE), defaultLinkDescription(dependency), gettingStartedSection::referenceDocs);
        registerLinks(indexedLinks.get(GuideType.GUIDE), defaultLinkDescription(dependency), gettingStartedSection::guides);
    ...
    private void registerLinks(List<Link> links, Function<List<Link>, String> defaultDescription, Supplier<BulletedSection<GettingStartedSection.Link>> section) {
        ...
        links.forEach((link) -> {
            if (link.getHref() != null) {
                String description = (link.getDescription() != null) ? link.getDescription() : defaultDescription.apply(links);
                if (description != null) {
