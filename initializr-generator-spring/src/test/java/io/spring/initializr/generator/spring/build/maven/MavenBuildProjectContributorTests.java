...
class MavenBuildProjectContributorTests {
    ...
    void pomIsContributedToProject() throws Exception {
        ...
        assertThat(lines).containsSequence("    <parent>", "        <groupId>org.springframework.boot</groupId>", "        <artifactId>spring-boot-starter-parent</artifactId>", "        <version>2.1.0.RELEASE</version>");
    }

    @Test
    void pomIsContributedUsingMavenContentId() throws Exception {
        IndentingWriterFactory indentingWriterFactory = IndentingWriterFactory.create(new SimpleIndentStrategy("    "), (factory) -> factory.indentingStrategy("maven", new SimpleIndentStrategy("\t")));
        MavenBuild build = new MavenBuild();
        build.settings().coordinates("com.example.demo", "demo").parent("org.springframework.boot", "spring-boot-starter-parent", "2.1.0.RELEASE");
