...
class GradleProjectGenerationConfigurationTests {
    ...
    void setup(@TempDir Path directory) {
        this.projectTester = new ProjectAssetTester().withIndentingWriterFactory().withConfiguration(BuildProjectGenerationConfiguration.class, GradleProjectGenerationConfiguration.class).withDirectory(directory).withBean(InitializrMetadata.class, () -> InitializrMetadataTestBuilder.withDefaults().build()).withDescriptionCustomizer((description) -> description.setBuildSystem(new GradleBuildSystem()));
    }
    ...
    void buildWriterIsContributed(String platformVersion) {
        ...
        this.projectTester.configure(description, (context) -> {
            assertThat(context).hasSingleBean(BuildWriter.class).getBean(BuildWriter.class).isInstanceOf(GradleBuildProjectContributor.class);
            assertThat(ReflectionTestUtils.getField(context.getBean(BuildWriter.class), "buildWriter")).isInstanceOf(GroovyDslGradleBuildWriter.class);
        });
    }

    static Stream<Arguments> gradleWrapperParameters() {
        return Stream.of(Arguments.arguments("2.2.3.RELEASE", "6.9.3"), Arguments.arguments("2.5.0", "7.6.1"));
    }
    ...
    void gradleWrapperIsContributedWhenGeneratingGradleProject(String platformVersion, String expectedGradleVersion) {
        ...
        assertThat(project).containsFiles("gradlew", "gradlew.bat", "gradle/wrapper/gradle-wrapper.properties", "gradle/wrapper/gradle-wrapper.jar");
        assertThat(project).textFile("gradle/wrapper/gradle-wrapper.properties").containsOnlyOnce(String.format("gradle-%s-bin.zip", expectedGradleVersion));
    }
    ...
    void buildDotGradleIsContributedWhenGeneratingGradleProject() {
        ...
        ProjectStructure project = this.projectTester.generate(description);
        assertThat(project).textFile("build.gradle").containsExactly(// @formatter:off
        "plugins {", "    id 'java'", "    id 'org.springframework.boot' version '2.4.0'", "    id 'io.spring.dependency-management' version '1.0.6.RELEASE'", "}", "", "group = 'com.example'", "version = '0.0.1-SNAPSHOT'", "sourceCompatibility = '11'", "", "repositories {", "    mavenCentral()", "}", "", "dependencies {", "    implementation 'org.springframework.boot:spring-boot-starter'", "    implementation 'com.example:acme'", "    testImplementation 'org.springframework.boot:spring-boot-starter-test'", "}", "", "tasks.named('test') {", "    useJUnitPlatform()", // @formatter:on
        "}");
    }
    ...
    void junitPlatformIsConfiguredWithCompatibleVersion() {
        ...
        description.setLanguage(new JavaLanguage());
        ProjectStructure project = this.projectTester.generate(description);
        assertThat(project).textFile("build.gradle").lines().containsSequence("tasks.named('test') {", "    useJUnitPlatform()", "}");
    }
    ...
    void testStarterExcludesVintageEngineWithCompatibleVersion() {
        ...
        description.setLanguage(new JavaLanguage());
        ProjectStructure project = this.projectTester.generate(description);
        assertThat(project).textFile("build.gradle").lines().containsSequence("    testImplementation('org.springframework.boot:spring-boot-starter-test') {", "        exclude group: 'org.junit.vintage', module: 'junit-vintage-engine'", "    }");
    }
    ...
    void gradleAnnotationProcessorScopeCustomizerIsContributed() {
        ...
        description.setPlatformVersion(Version.parse("2.4.0"));
        description.setLanguage(new JavaLanguage());
        this.projectTester.configure(description, (context) -> assertThat(context).hasSingleBean(GradleAnnotationProcessorScopeBuildCustomizer.class));
    }
}
