...
class GradleKtsProjectGenerationConfigurationTests {
    ...
    void setup(@TempDir Path directory) {
        this.projectTester = new ProjectAssetTester().withIndentingWriterFactory().withConfiguration(BuildProjectGenerationConfiguration.class, GradleProjectGenerationConfiguration.class).withDirectory(directory).withBean(InitializrMetadata.class, () -> InitializrMetadataTestBuilder.withDefaults().build()).withDescriptionCustomizer((description) -> description.setBuildSystem(new GradleBuildSystem(GradleBuildSystem.DIALECT_KOTLIN)));
    }
    ...
    void buildWriterIsContributed(String platformVersion) {
        ...
        this.projectTester.configure(description, (context) -> {
            ...
            assertThat(ReflectionTestUtils.getField(context.getBean(BuildWriter.class), "buildWriter")).isInstanceOf(KotlinDslGradleBuildWriter.class);
        });
    }
    ...
    void junitPlatformIsConfiguredWithCompatibleVersion() {
        ...
        description.setLanguage(new JavaLanguage());
        ProjectStructure project = this.projectTester.generate(description);
        assertThat(project).textFile("build.gradle.kts").lines().containsSequence("tasks.withType<Test> {", "    useJUnitPlatform()", "}");
    }
}
