...
public abstract class AbstractInitializrIntegrationTests {
    ...
    protected void validateContentType(ResponseEntity<String> response, MediaType expected) {
        ...
        assertThat(actual.isCompatibleWith(expected)).as("Non compatible media-type, expected " + expected + ", got " + actual).isTrue();
    }
    ...
    protected void validateMetadata(ResponseEntity<String> response, MediaType mediaType, String version, JSONCompareMode compareMode) {
        try {
    ...
    protected void assertDefaultJavaProject(ProjectStructure project) {
        assertThat(project).containsFiles("src/main/java/com/example/demo/DemoApplication.java", "src/test/java/com/example/demo/DemoApplicationTests.java", "src/main/resources/application.properties");
    }
    ...
    protected <T> ResponseEntity<T> execute(String contextPath, Class<T> responseType, String userAgentHeader, String... acceptHeaders) {
        ...
        return this.restTemplate.exchange(createUrl(contextPath), HttpMethod.GET, new HttpEntity<Void>(headers), responseType);
    }
    ...
    private Set<PosixFilePermission> getPosixFilePermissions(int unixMode) {
        return Arrays.stream(BitMaskFilePermission.values()).filter((permission) -> permission.permitted(unixMode)).map(BitMaskFilePermission::getFilePermission).collect(Collectors.toSet());
    }

    private void applyPermissions(Path target, Set<PosixFilePermission> permissions) throws IOException {
        if (isWindows()) {
            File file = target.toFile();
            applyPermission(file::setReadable, permissions, PosixFilePermission.OWNER_READ, PosixFilePermission.GROUP_READ, PosixFilePermission.OTHERS_READ);
            applyPermission(file::setWritable, permissions, PosixFilePermission.OWNER_WRITE, PosixFilePermission.GROUP_WRITE, PosixFilePermission.OTHERS_WRITE);
            applyPermission(file::setExecutable, permissions, PosixFilePermission.OWNER_EXECUTE, PosixFilePermission.GROUP_EXECUTE, PosixFilePermission.OTHERS_EXECUTE);
        } else {
            Files.setPosixFilePermissions(target, permissions);
        }
    }

    private void applyPermission(BiConsumer<Boolean, Boolean> target, Set<PosixFilePermission> permissions, PosixFilePermission ownerPermission, PosixFilePermission... nonOwnerPermissions) {
